 ...
 */
function startupgrowth_commerce_install_tasks_alter(&$tasks, $install_state) {
	...
	$tasks['startupgrowth_commerce_content'] = array(
 ...
 */
function startupgrowth_commerce_form_install_configure_form_alter(&$form, $form_state) {
	$form['site_information']['site_name']['#default_value'] = 'Startup Growth';
    $form['#submit'][] = 'startupgrowth_commerce_install_configure_form_submit';
}

/**
 * Forms API submit for the site configuration form.
 */
function startupgrowth_commerce_install_configure_form_submit($form, &$form_state) {
	global $user;
...
function startupgrowth_commerce_content(&$install_state) {
	$sql_file=dirname(__FILE__).'/startupgrowth_commerce.sql';
	$count=startupgrowth_commerce_import_sql($sql_file);
}

function startupgrowth_commerce_import_sql($filename){
